    ...
    while running:
        ...
        if game_started and goal_hexagram and ROUND_OPTIMAL_DIST is not None and not POPUP_VISIBLE:
            ...
            if 0 <= opt_index < total_slots:
                ...
                # color logic (incl. yellow when perfectly optimal)
                made_optimal = (
                    SEQ_OUTCOME == "success"
                    and ROUND_OPTIMAL_DIST is not None
                    and len(hexagram_chain) - 1 == ROUND_OPTIMAL_DIST
                )
                filled_and_wrong = (
                    len(hexagram_chain) > opt_index and
                    hexagram_chain[opt_index]["binary"] != goal_hexagram["binary"]
                )
                if made_optimal:
                    opt_color = (255, 255, 0)
                elif filled_and_wrong:
                    opt_color = (235, 120, 120)
                else:
                    opt_color = (255, 255, 255)
    
                # Build OUTER slot rect (no padding) for the OPTIMAL slot
                ...
                # Render label and place it JUST above the outer slot (so it never sits inside the card)
                tag = render_surf(font, "↓ OPTIMAL ↓", opt_color)
                tag_rect = tag.get_rect(
                    midbottom=(slot_outer.centerx, slot_outer.top - OPT_LABEL_GAP)
                )
    
                # Keep it inside the grid vertically
                if tag_rect.top < 0:
                    tag_rect.top = 0
    
                screen.blit(tag, tag_rect)
    
                # NEW: strikethrough once the optimal slot has been passed (turns red)
                if filled_and_wrong:
                    optimal_filled_wrong = True
                    y = tag_rect.centery  # strike across the middle of the text
                    pad = 4               # extend a hair past the text bounds
                    pygame.draw.line(
                        screen,
                        opt_color,                      # same red you’re using for the label
                        (tag_rect.left - pad, y),
        ...
        if game_started and goal_hexagram and not POPUP_VISIBLE:
            # Show as soon as OPTIMAL is crossed out (i.e., OPTIMAL slot is filled but wrong)
            if (ROUND_OPTIMAL_DIST is not None):
                # We rely on 'filled_and_wrong' computed in the OPTIMAL block above.
                # If you moved this block elsewhere, recompute it like you do there.
                if optimal_filled_wrong:
                    moves_so_far = len(hexagram_chain) - 1
    
                    # Remaining moves are based on the ROUND LIMIT, not grid columns
                    remaining = max(0, transformation_limit - moves_so_far)
    
                    # Live shortest distance from CURRENT card (already updated by recompute_live_guidance)
                    dist = LIVE_POSSIBLE_DIST
    
                    # Determine state and target index
                    impossible = (dist is None) or (dist > remaining)
                    if impossible:
                        poss_text  = "↓ NOT POSSIBLE ↓"
                        poss_color = (235, 120, 120)  # red
                        target_index = transformation_limit          # freeze at the last allowed slot (10th)
                    else:
                        poss_text  = "↓ POSSIBLE ↓"
                        poss_color = (230, 230, 230)
                        target_index = moves_so_far + dist
    
                        # NEW: if we’ve just achieved the goal, color like OPTIMAL’s “perfect” yellow
                        if SEQ_OUTCOME == "success" and dist == 0:
                            poss_color = (255, 255, 0)   # same yellow as OPTIMAL-on-perfect
    
                    # Map target_index -> row/col using your grid geometry
                    ...
                    # Place the label just like OPTIMAL (above the slot, with your gap)
